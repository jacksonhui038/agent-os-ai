...
(function() {
  ...
  const MULTI_PLATFORMS = [
    ...
    { key: 'dy',  name: '抖音',         dims: { w: 1080, h: 1920 }, ratioTxt: '9:16 直屏' },
    { key: 'th',  name: 'Threads',       dims: { w: 1080, h: 1080 }, ratioTxt: '1:1 方形' }
  ];
  let lastMulti = null; // 記低上次一鍵多平台參數，供 langToggle 即時重渲染
  ...
  // 平台專屬 caption（長短 / 語氣按平台調整；支援招聘模式 + 內地簡中雙版）
  function buildPlatformCaption(topic, key, style, persona, extra, audience, rich) {
    ...
    const baseTags = rich.match ? T.socialTopics[rich.match].hashtags : suggestTags(topic, key === 'xhs' ? 'xhs' : key);
    const points = rich.points;
    const intro = '好多朋友都有呢個疑問，我整理咗重點同你分享：';
    const tip = '💡 小貼士：以上係一般資訊，實際方案要睇你嘅家庭狀況同預算，歡迎 PM 我免費分析。';

    // 招聘模式（招攬同事）：切換 contentMode = recruit
    const isRecruit = document.getElementById('contentMode') && document.getElementById('contentMode').value === 'recruit';
    const hook = isRecruit ? '【誠邀加盟】' + rich.hook : rich.hook;
    const cta = isRecruit ? '有興趣做保險顧問？歡迎 PM 我了解入行／轉職嘅嘢 🙌' : rich.cta;
    const recruitTags = '#香港保險 #保險招聘 #理財顧問 #職業機會 #JoinUs';
    const tags = isRecruit ? recruitTags : baseTags;

    // 內地簡中雙版：langToggle 切 cn-mode 時，全平台文案轉簡體
    const cn = document.getElementById('langToggle') && document.getElementById('langToggle').classList.contains('cn-mode');
    function toCN(s) {
      if (!cn) return s;
    ...
    // dy（抖音）：短句＋emoji密，直屏 9:16 配合
    if (key === 'dy') {
      return toCN(`🔥 ${hook}\n\n${points.map((p, i) => `${i + 1}. ${p}`).join('\n')}\n\n${cta}\n\n${tags} #香港保險 #理財乾貨`);
    }
    // th（Threads）：類 IG 但更對話式
    if (key === 'th') {
      return toCN(`🤔 ${hook}\n\n分享幾個重點：\n\n${points.map(p => '→ ' + p).join('\n')}\n\n有問題隨時問 👇\n\n${cta}\n\n${tags}`);
    }
    // fb：專業段落
  ...
  async function generateMultiPlatform(opts) {
    ...
    let html = `<div class="dup-warn" style="background:#eef2ff;border-color:#6366f1;color:#3730a3">🚀 已一次過生成 <b>Instagram / 小紅書 / Facebook / 抖音 / Threads</b> 五個平台版本，各自獨立尺寸同文案長短。每張圖下有「⬇️ 圖」同「🎨 Canva」。</div><div class="mp-grid">`;
  ...
  function openMultiInCanva(key) {
    const dims = { ig: { w: 1080, h: 1080 }, xhs: { w: 1080, h: 1440 }, fb: { w: 1200, h: 630 }, reels: { w: 1080, h: 1920 }, dy: { w: 1080, h: 1920 }, th: { w: 1080, h: 1080 } }[key] || { w: 1080, h: 1080 };
  ...
  function copyAllMulti() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(text).then(() => alert('已複製全部 ' + MULTI_PLATFORMS.length + ' 個平台文案')).catch(() => fallbackCopy(text));
  ...
  // 詞組級（先處理，避免被逐字破壞）
  const SIMP_PHRASES = [
    [/留言/g, '评论'], [/私訊/g, '私信'], [/小紅書/g, '小红书'], [/理財/g, '理财'],
    [/保險/g, '保险'], [/筆記/g, '笔记'], [/乾貨/g, '干货'], [/獲客/g, '获客'],
    [/轉職/g, '转职'], [/歡迎/g, '欢迎'], [/專頁/g, '专页'], [/粉絲/g, '粉丝'],
    [/關注/g, '关注'], [/連結/g, '链接'], [/點擊/g, '点击'], [/查詢/g, '查询'],
    [/預約/g, '预约'], [/計劃/g, '计划'], [/醫療/g, '医疗'], [/危疾/g, '重疾'],
    [/儲蓄/g, '储蓄'], [/職位/g, '职位'], [/團隊/g, '团队'], [/客戶/g, '客户'],
    [/顧問/g, '顾问'], [/建議/g, '建议'], [/準備/g, '准备'], [/選擇/g, '选择'],
    [/開戶/g, '开户'], [/轉帳/g, '转账'], [/資訊/g, '信息'], [/帳戶/g, '账户'],
    [/賬戶/g, '账户'], [/服務/g, '服务'], [/資料/g, '资料'], [/聯絡/g, '联络'],
    [/證件/g, '证件'], [/稅務/g, '税务'], [/網頁/g, '网页'], [/網站/g, '网站'],
    [/訂閱/g, '订阅'], [/購買/g, '购买'], [/讀取/g, '读取'], [/頁面/g, '页面'],
    [/開放/g, '开放'], [/發送/g, '发送'], [/發布/g, '发布'], [/發現/g, '发现'],
    [/處理/g, '处理'], [/圖書/g, '图书'], [/學習/g, '学习'], [/說明/g, '说明'],
    [/評論/g, '评论'], [/應該/g, '应该'], [/時候/g, '时候'], [/會計/g, '会计'],
    [/內容/g, '内容'], [/資產/g, '资产'], [/負債/g, '负债'], [/優惠/g, '优惠'],
    [/稅收/g, '税收'], [/稅率/g, '税率'], [/結餘/g, '结余'], [/餘額/g, '余额'],
    [/續保/g, '续保'], [/賠償/g, '赔偿'], [/索償/g, '索赔'], [/保單/g, '保单'],
    [/投保/g, '投保'], [/受保/g, '受保'], [/供款/g, '供款'], [/復效/g, '复效'],
    [/終身/g, '终身'], [/週年/g, '周年'], [/週期/g, '周期'], [/線上/g, '线上'],
    [/線下/g, '线下'], [/過戶/g, '过户'], [/確認/g, '确认'], [/詳細/g, '详细'],
    [/聯繫/g, '联系'], [/電郵/g, '电邮'], [/郵件/g, '邮件'], [/帳單/g, '账单'],
    [/錢包/g, '钱包'], [/銀行/g, '银行'], [/貸款/g, '贷款'], [/債務/g, '债务']
  ];
  // 逐字級
  const SIMP_CHARS = {
    '險': '险', '資': '资', '訊': '讯', '帳': '账', '賬': '账', '戶': '户', '務': '务',
    '點': '点', '擊': '击', '關': '关', '注': '注', '連': '连', '結': '结', '網': '网',
    '頁': '页', '購': '购', '買': '买', '讀': '读', '學': '学', '習': '习', '說': '说',
    '評': '评', '論': '论', '應': '应', '該': '该', '時': '时', '會': '会', '計': '计',
    '發': '发', '現': '现', '處': '处', '圖': '图', '書': '书', '開': '开', '訂': '订',
    '閱': '阅', '查': '查', '詢': '询', '預': '预', '約': '约', '劃': '划', '醫': '医',
    '療': '疗', '危': '危', '疾': '疾', '儲': '储', '蓄': '蓄', '職': '职', '團': '团',
    '隊': '队', '客': '客', '顧': '顾', '問': '问', '建': '建', '議': '议', '準': '准',
    '備': '备', '擇': '择', '選': '选', '轉': '转', '專': '专', '豐': '丰', '銷': '销',
    '費': '费', '錢': '钱', '銀': '银', '話': '话', '語': '语', '體': '体', '腦': '脑',
    '電': '电', '認': '认', '識': '识', '護': '护', '車': '车', '馬': '马', '魚': '鱼',
    '鳥': '鸟', '門': '门', '間': '间', '閉': '闭', '鐵': '铁', '銅': '铜', '針': '针',
    '錯': '错', '鐘': '钟', '鏡': '镜', '鑽': '钻', '鎖': '锁', '釘': '钉', '鋼': '钢',
    '鈴': '铃', '鍵': '键', '鈔': '钞', '錄': '录', '鎮': '镇', '長': '长', '們': '们',
    '來': '来', '這': '这', '那': '那', '與': '与', '為': '为', '當': '当', '實': '实',
    '線': '线', '東': '东', '親': '亲', '見': '见', '對': '对', '個': '个', '後': '后',
    '兩': '两', '題': '题', '廳': '厅', '廠': '厂', '廣': '广', '場': '场', '號': '号',
    '覺': '觉', '覽': '览', '規': '规', '則': '则', '補': '补', '額': '额', '類': '类',
    '驗': '验', '證': '证', '譽': '誉', '變': '变', '態': '态', '總': '总', '紅': '红',
    '綠': '绿', '級': '级', '統': '统', '紙': '纸', '維': '维', '織': '织', '經': '经',
    '濟': '济', '財': '财', '稅': '税', '確': '确', '輕': '轻', '運': '运', '輸': '输',
    '過': '过', '進': '进', '還': '还', '風': '风', '飛': '飞', '雲': '云', '業': '业',
    '萬': '万', '歷': '历', '歲': '岁', '縣': '县', '農': '农', '義': '义', '禮': '礼',
    '視': '视', '試': '试', '課': '课', '許': '许', '誠': '诚', '請': '请', '誰': '谁',
    '顯': '显', '豐': '丰', '滅': '灭', '營': '营', '環': '环', '護': '护', '獲': '获',
    '舊': '旧', '張': '张', '強': '强', '書': '书', '陸': '陆', '愛': '爱', '礙': '碍',
    '爭': '争', '產': '产', '責': '责', '贊': '赞', '遞': '递', '遷': '迁', '達': '达',
    '遠': '远', '邊': '边', '辦': '办', '勵': '励', '參': '参', '觀': '观', '覺': '觉',
    '繼': '继', '續': '续', '緣': '缘', '罰': '罚', '罷': '罢', '習': '习', '聖': '圣',
    '藝': '艺', '蘋': '苹', '藥': '药', '處': '处', '複': '复', '見': '见', '親': '亲',
    '覺': '觉', '讓': '让', '禮': '礼', '體': '体', '質': '质', '敵': '敌', '讀': '读',
    '賣': '卖', '買': '买', '負': '负', '責': '责', '賺': '赚', '輸': '输', '贏': '赢',
    '軍': '军', '農': '农', '邁': '迈', '運': '运', '還': '还', '週': '周', '種': '种',
    '積': '积', '穩': '稳', '窩': '窝', '窮': '穷', '離': '离', '麥': '麦', '龍': '龙',
    '傳': '传', '傷': '伤', '價': '价', '眾': '众', '優': '优', '僅': '仅', '付': '付',
    '傑': '杰', '備': '备', '勝': '胜', '勞': '劳', '協': '协', '單': '单', '嘆': '叹',
    '響': '响', '報': '报', '園': '园', '圓': '圆', '圖': '图', '塊': '块', '聲': '声',
    '壞': '坏', '夢': '梦', '妝': '妆', '孫': '孙', '實': '实', '審': '审', '對': '对',
    '尋': '寻', '將': '将', '爾': '尔', '盡': '尽', '巖': '岩', '幫': '帮', '廣': '广',
    '後': '后', '徹': '彻', '慮': '虑', '態': '态', '慶': '庆', '據': '据', '擔': '担',
    '擊': '击', '敵': '敌', '數': '数', '斷': '断', '暫': '暂', '曉': '晓', '會': '会',
    '極': '极', '構': '构', '權': '权', '歡': '欢', '殺': '杀', '氣': '气', '氫': '氢',
    '歸': '归', '歲': '岁', '殘': '残', '殺': '杀', '沒': '没', '溝': '沟', '滅': '灭',
    '漸': '渐', '熱': '热', '爭': '争', '獎': '奖', '瑪': '玛', '產': '产', '畫': '画',
    '異': '异', '當': '当', '瘋': '疯', '監': '监', '盤': '盘', '睞': '睐', '短': '短',
    '砲': '炮', '礦': '矿', '禮': '礼', '種': '种', '穫': '获', '級': '级', '納': '纳',
    '純': '纯', '細': '细', '終': '终', '習': '习', '聯': '联', '臉': '脸', '臨': '临',
    '與': '与', '興': '兴', '舊': '旧', '蒐': '搜', '處': '处', '號': '号', '術': '术',
    '衝': '冲', '裝': '装', '製': '制', '認': '认', '誤': '误', '課': '课', '調': '调',
    '請': '请', '論': '论', '質': '质', '賢': '贤', '賣': '卖', '購': '购', '賴': '赖',
    '贈': '赠', '輸': '输', '遲': '迟', '醫': '医', '錶': '表', '錯': '错', '鍊': '炼',
    '鍵': '键', '鍾': '钟', '錢': '钱', '錦': '锦', '靜': '静', '領': '领', '頭': '头',
    '題': '题', '風': '风', '飛': '飞', '飲': '饮', '養': '养', '鳥': '鸟', '麗': '丽',
    '勳': '勋', '勵': '励', '參': '参', '雙': '双', '葉': '叶', '夢': '梦', '歲': '岁',
    '產': '产', '聲': '声', '畫': '画', '戰': '战', '擁': '拥', '據': '据', '歸': '归',
    '滅': '灭', '無': '无', '點': '点', '為': '为', '當': '当', '尋': '寻', '將': '将',
    '爾': '尔', '盡': '尽', '爭': '争', '為': '为', '這': '这', '個': '个', '來': '来',
    '們': '们', '對': '对', '實': '实', '後': '后', '兩': '两', '與': '与', '為': '为',
    '當': '当', '實': '实', '線': '线', '東': '东', '親': '亲', '見': '见', '題': '题'
  };
  function toSimplified(s) {
    if (!s) return s;
    let out = s;
    for (const [re, rep] of SIMP_PHRASES) out = out.replace(re, rep);
    let r = '';
  ...
  function scanCurrentOutput() {
    const parts = [];
    ['mpCap_ig','mpCap_xhs','mpCap_fb','mpCap_dy','mpCap_th'].forEach(id => { const e = document.getElementById(id); if (e) parts.push(e.textContent); });
    const text = parts.join('\n\n');
  ...
  function topicBucket(topic) {
    const t = (topic || '').toLowerCase();
    if (/小紅書|抖音|社媒|社交|獲客|引流/.test(t)) return 'xhs';
    if (/客戶|獲客|開單|銷售|業績/.test(t)) return 'client';
    if (/重疾|危疾|醫療|健康|保病|住院|自願醫保|vhis/.test(t)) return 'medical';
    if (/儲蓄|理財|分紅|投資|儲錢|教育金|退休/.test(t)) return 'savings';
    if (/ai|自動|文案|出 post|出片/.test(t)) return 'ai';
    return 'default';
  }
  function chooseReelsTitle(topic) {
    const bucket = topicBucket(topic);
    const list = REELS_TITLE_TEMPLATES[bucket] || REELS_TITLE_TEMPLATES.default;
    const clean = (topic || '').replace(/[。，、！？]/g, '').trim();
    let idx = 0;
    for (let i = 0; i < clean.length; i++) idx += clean.charCodeAt(i);
