// ===== Proposal.js — Proposal 引擎 v2（Wizard 工作台） =====
(function() {
  let selectedProducts = [];
  let currentStep = 1;
  const NEEDS_MAP = {
    health: ['medical'], ci: ['ci'], life: ['life'],
    retire: ['retirement'], save: ['savings'], tax: ['vhis']
  };
  ...
  function goStep(n) {
    ...
    // Hide all panels
    for (let i = 1; i <= 4; i++) {
      var p = document.getElementById('wizPanel' + i);
    ...
    // Update lines
    document.querySelectorAll('.wiz-line').forEach(function(el) {
      var l = parseInt(el.dataset.line);
      el.style.background = l < n ? 'var(--primary)' : 'var(--border)';
    });
    currentStep = n;
  }

  function validateStep1() {
    var sel = document.getElementById('propClientSelect').value;
    var nc = (document.getElementById('propNewClient').value || '').trim();
    if (!sel && !nc) { alert('請選擇客戶或輸入新客戶名稱'); return false; }
    return true;
  }

  function validateStep2() { return true; } // needs are optional, budget optional

  function onClientSelect() {
    var id = document.getElementById('propClientSelect').value;
    if (id && window._lastAnalyzedClient && window._lastAnalyzedClient.id === id) {
      var c = window._lastAnalyzedClient;
      var t = document.getElementById('propTitle');
      if (t && !t.value.trim()) t.value = c.name + ' 保障建議書';
    }
  }
  ...
  function resetWizard() {
    ...
    selectedProducts = [];
    goStep(1);
    document.getElementById('proposalOutput').className = 'output-box';
    document.getElementById('proposalOutput').innerHTML = '';
    document.querySelectorAll('.prop-needs-chip').forEach(function(el,i){ el.classList.toggle('active',i<2); });
    document.querySelectorAll('.prop-chip.active').forEach(function(el){ el.classList.remove('active'); });
  }

  function getSelectedNeeds() {
    var arr = [];
    document.querySelectorAll('.prop-needs-chip.active').forEach(function(el){ arr.push(el.dataset.val); });
    return arr;
  }
  ...
  function buildSummary() {
    ...
    var ids = selectedProducts.length ? selectedProducts : Array.from(document.querySelectorAll('.prop-chip.active')).map(function(e){return e.dataset.val;});
    var products = ids.map(function(id){return window.PRODUCTS[id];}).filter(Boolean);
    var summary = document.getElementById('proposalSummary');
    if (!summary) return;
    summary.innerHTML = '<strong>👤 客戶：</strong>' + (client ? client.name : document.getElementById('propNewClient').value || '-') +
      '<br><strong>🎯 需求：</strong>' + (needs.length ? needs.join('、') : '-').replace(/health/g,'醫療').replace(/ci/g,'危疾').replace(/life/g,'人壽').replace(/retire/g,'退休').replace(/save/g,'儲蓄').replace(/tax/g,'稅務') +
      '<br><strong>📦 產品：</strong>' + (products.length ? products.map(function(p){return p.name}).join('、') : '未揀') +
      '<br><strong>🎨 風格：</strong>' + (getStyleLabel()) + '｜<strong>詳細度：</strong>' + (document.getElementById('propDepth')||{}).value || '-';
  }

  function getStyleLabel() {
    var el = document.querySelector('.prop-style-chip.active');
    var map = {advisor:'專業顧問式',data:'科技數據風',warm:'明亮親和',business:'簡潔商務'};
    return el ? (map[el.dataset.val] || el.dataset.val) : '-';
  }
  ...
  function attachNeedsChips() {
    document.querySelectorAll('.prop-needs-chip').forEach(el => {
  ...
  function getClient() {
    ...
    // 新客戶輸入
    if (!client) {
      const nc = (document.getElementById('propNewClient') || {}).value || '';
      if (nc.trim()) client = { name: nc.trim(), age: '', job: '', marital: '', kids: 0, income: 0 };
    }
    return client;
  }
  ...
  function renderProposal(client, products, depth, audience, title, styleMeta) {
    ...
    const styleIntro = {
      ...
      business: '以下是根據您的保障需求所制定的配置方案，重點摘要如下。'
    };
