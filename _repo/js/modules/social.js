...
(function() {
  ...
  function buildPlatformCaption(topic, key, style, persona, extra, audience, rich) {
    ...
    function toCN(s) {
      if (!cn) return s;
      return s
        .replace(/保險/g, '保险').replace(/理財/g, '理财').replace(/小紅書/g, '小红书')
        .replace(/留言/g, '评论').replace(/私訊/g, '私信').replace(/歡迎/g, '欢迎')
        .replace(/筆記/g, '笔记').replace(/獲客/g, '获客').replace(/轉職/g, '转职');
    }
