...
(async () => {
  ...
  const capIds = ['mpCap_ig', 'mpCap_xhs', 'mpCap_fb', 'mpCap_dy', 'mpCap_th'];
  const caps = capIds.map(id => window.document.getElementById(id));
  let ok = true;
  if (!r.ok) { console.log('❌ generateMultiPlatform 返 ok=false'); ok = false; }
  if (canvases.length !== 5) { console.log('❌ canvas 數目 =', canvases.length, '(期望 5)'); ok = false; }
  caps.forEach((c, i) => { if (!c || !c.textContent.trim()) { console.log('❌ caption', capIds[i], '空'); ok = false; } });
  // 平台 caption 長短應有分別（xhs 最長）
  const len = caps.map(c => c.textContent.length);
  if (!(len[1] >= Math.max(...len))) { console.log('❌ xhs caption 應最長', len); ok = false; }
  if (ok) console.log('✅ generateMultiPlatform OK：5 canvas + 5 caption，xhs 最長', len);
  else process.exit(1);
})();
