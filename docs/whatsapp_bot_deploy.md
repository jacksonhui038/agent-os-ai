...
# Phase 2（可選）：設咗就直接 send 圖，唔使 send link
...
## E. Phase 2：Bot 直接 send 圖（唔使 send link）

Edge Function（Deno）冇 DOM，唔可以直接 render Canvas，所以加咗一個 `render-service/`
（Node + Puppeteer headless Chrome）將 `render.html` 轉 PNG。部署詳見 `render-service/README.md`。

流程：
```
客戶 WhatsApp → Meta Webhook → whatsapp-bot Edge Function
                                    ├─ callLLM 分類（query/command）
                                    ├─ 組 render.html?... link
                                    ├─ call RENDER_SERVICE_URL?url=<link> 拎 PNG
                                    ├─ Graph API 上傳 media + send 張圖（caption = AI 回覆）
                                    └─ 失敗自動 fallback 返 send link
```

部署步驟：
```bash
# 1. 部署 render-service（Railway / Render / Fly，見 render-service/README.md）
#    拎到 URL，例如 https://agent-os-render.up.railway.app/render

# 2. 設 RENDER_SERVICE_URL secret
supabase secrets set RENDER_SERVICE_URL=https://agent-os-render.up.railway.app/render
...
唔設 `RENDER_SERVICE_URL` 就會保持原本 send link 行為，向後兼容。
